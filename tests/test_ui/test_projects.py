...
def test_add_new_project(browser_config_ui, auth):
...
def test_try_add_empty_name_project(browser_config_ui, auth):
...
def test_edit_project(browser_config_ui, auth):
...
def test_check_all_new_project_tabs(browser_config_ui, auth):
    with allure.step("Go to main page"):
